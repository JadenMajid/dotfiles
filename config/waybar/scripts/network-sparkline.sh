...
if [[ -z "$STATS" ]]; then
  ...
  # ifstat outputs KB/s. Let's convert for display.
  read UP_KB DOWN_KB <<<"$STATS"
  
  if (( $(echo "$DOWN_KB > 1024" | bc -l) )); then
      DOWN=$(awk "BEGIN {printf \"%.1fM\", $DOWN_KB/1024}")
  else
      DOWN=$(awk "BEGIN {printf \"%.0fK\", $DOWN_KB}")
  fi

  if (( $(echo "$UP_KB > 1024" | bc -l) )); then
      UP=$(awk "BEGIN {printf \"%.1fM\", $UP_KB/1024}")
  else
      UP=$(awk "BEGIN {printf \"%.0fK\", $UP_KB}")
  fi
  
  # Keep raw values for sparkline
  DOWN_VAL=$DOWN_KB
  UP_VAL=$UP_KB
fi

# Update history
HISTORY_DOWN+=($DOWN_VAL)
HISTORY_UP+=($UP_VAL)
...
# Escape tooltip
ESCAPED_TOOLTIP=$(printf 'Down: %s\n%s\n\nUp: %s\n%s' "$DOWN" "$SPARKLINE_DOWN" "$UP" "$SPARKLINE_UP" | python3 -c 'import json,sys; print(json.dumps(sys.stdin.read()))')

# Output JSON with fixed width padding (approx 5 chars for the speed)
printf '{"text": "%5s  %5s ", "tooltip": %s}\n' "$DOWN" "$UP" "$ESCAPED_TOOLTIP"
